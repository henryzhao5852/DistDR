...
def main(args):
    ...
    for data in dataset:
        ...
        question_answers.append(data['answers'])
    
    print(len(questions))
    top_ids_and_scores = list()
    num_iters = int(len(questions) / 10000) + 1 
    print(num_iters)
    for i in range(num_iters):
        start = time.time()
        questions_tensor = retriever.generate_question_vectors(questions[i * 10000: (i + 1) * 10000])
        print(questions_tensor.size(), i)
        # get top k results
        ids_and_scores = retriever.get_top_docs(questions_tensor.numpy(), args.n_docs)
        top_ids_and_scores.extend(ids_and_scores)
        print(time.time() - start)

    print(len(ids_and_scores))
    ...
    if args.out_file:
        out_file = args.out_file + '.json'
        save_results(all_passages, questions, question_answers, top_ids_and_scores, questions_doc_hits, out_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--qa_file', required=False, type=str, default=None,
                        help="Question and answers file")
    ...
    parser.add_argument("--num", type=int, help='num of splits')
